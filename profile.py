...
def flow(c, h, d, u_inf, alpha):
	...
	plt.contour(Z_xmesh, Z_ymesh, Psi_phys, levels=100, cmap='viridis') #Solution in parametric plane, shifted to t plane.
...
def process_argv(argv, da):
	...
	if (argc>2):
		for i_arg in range(argc):
			...
			if (argv[i_arg] == "c"):
				da["c"] = int(argv[i_arg+1])
			if (argv[i_arg] == "h"):
				da["h"] = int(argv[i_arg+1])
			if (argv[i_arg] == "d"):
				da["d"] = int(argv[i_arg+1])
		return 0
	return 1
